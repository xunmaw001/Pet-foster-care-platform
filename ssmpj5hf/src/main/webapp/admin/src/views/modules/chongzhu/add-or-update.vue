...
export default {
  data() {
	...
	lianxidianhua : false,
      ...
      rules: {
          chongzhuzhanghao: [
                { required: true, message: '宠主账号不能为空', trigger: 'blur' },
          ],
          chongzhuxingming: [
                { required: true, message: '宠主姓名不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chongzhu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chongzhuCrossAddOrUpdateFlag = false;
						 this.parent.search();
