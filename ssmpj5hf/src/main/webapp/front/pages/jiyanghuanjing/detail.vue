      	...
      	<swiper-item :style='{"padding":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"332rpx"}' v-for="(swiper,index) in swiperList" :key="index">
			...
			<view class="price detail-list-item" :style='{"padding":"0","boxShadow":"0 0 4rpx rgba(0,0,0,.3)","margin":"0 0 0px 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
                ...
                <view class="name shop-title" :style='{"padding":"24rpx","boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0","borderColor":"#fff","backgroundColor":"#fff","color":"#000","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"auto","fontSize":"32rpx","borderStyle":"solid"}'>
					区域名称：{{detail.quyumingcheng}}
				</view>
                <view class="name shop-title" :style='{"padding":"24rpx","boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0","borderColor":"#fff","backgroundColor":"#fff","color":"#000","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"auto","fontSize":"32rpx","borderStyle":"solid"}'>
					托管费用：{{detail.tuoguanfeiyong}}
				</view>
				...
				<view class="detail-list-item" :style='{"padding":"0","boxShadow":"0 0 4rpx rgba(0,0,0,.3)","margin":"0 0 0px 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"160rpx","lineHeight":"80rpx","fontSize":"28rpx","borderStyle":"solid"}'>消毒次数：</view>
					<view  class="text" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"calc(100% - 160rpx)","lineHeight":"80rpx","fontSize":"28rpx","borderStyle":"solid"}'>{{detail.xiaoducishu}}</view>
				</view>
				<view class="detail-list-item" :style='{"padding":"0","boxShadow":"0 0 4rpx rgba(0,0,0,.3)","margin":"0 0 0px 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"160rpx","lineHeight":"80rpx","fontSize":"28rpx","borderStyle":"solid"}'>面积：</view>
					<view  class="text" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"calc(100% - 160rpx)","lineHeight":"80rpx","fontSize":"28rpx","borderStyle":"solid"}'>{{detail.mianji}}</view>
				</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop" :style='{"padding":"0px","boxShadow":"rgb(0 0 0 / 0%) 0px 0px 12rpx","margin":"0px","borderColor":"rgb(204, 204, 204)","backgroundColor":"rgb(255, 255, 255)","borderRadius":"0px","borderWidth":"2rpx 0 0 0","width":"100%","borderStyle":"solid","height":"100rpx"}'>
				...
				<view style="text-align: right;">
					<button v-if="userid&&isAuth('jiyanghuanjing','寄养')" @tap="onAcrossTap('chongwujiyang','','','')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'24rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">寄养</button>
					<button v-if="!userid&&isAuthFront('jiyanghuanjing','寄养')" @tap="onAcrossTap('chongwujiyang','','','')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'24rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">寄养</button>
                </view>
			</view>
		</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discussjiyanghuanjingCleanType')
			if(cleanType){
				uni.removeStorageSync('discussjiyanghuanjingCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jiyanghuanjing');
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.quyumingcheng,
								name: _this.detail.tuoguanfeiyong,
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','jiyanghuanjing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('jiyanghuanjing', this.id);
				this.detail = res.data;
				// 轮播图片
				this.swiperList = this.detail.huanjing ? this.detail.huanjing.split(",") : [];
				//修改富文本的图片样式
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussjiyanghuanjing', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjiyanghuanjing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('jiyanghuanjing', this.detail);
				this.$utils.msg('审核成功');
